...
resource "azurerm_subnet" "public" {
  ...
  address_prefixes     = [cidrsubnet(one(azurerm_virtual_network.sec.address_space), 4, 0)]
...
resource "azurerm_subnet" "private" {
  ...
  resource_group_name  = azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.sec.name
  address_prefixes     = [cidrsubnet(one(azurerm_virtual_network.sec.address_space), 4, 1)]
